...
UPDATE_ALL_LAUNCHER_MD5="23630ec0665e115e06701db8cc60f87b"
...
MISTER_DEVEL_UPDATER_URL="https://raw.githubusercontent.com/theypsilon/Deprecated_Updater_script_MiSTer/master/mister_updater.sh"
MISTER_DEVEL_LINUX_ONLY_UPDATER_URL="https://raw.githubusercontent.com/theypsilon/Deprecated_Updater_script_Linux_Only_MiSTer/master/mister_updater.sh"
...
initialize() {
    ...
    if [[ "${DOWNLOADER_WHEN_POSSIBLE}" == "true" ]] && [[ "${UPDATE_ALL_PC_UPDATER}" != "true" ]] && [[ "${AUTO_UPDATE_LAUNCHER}" == "true" ]] ; then
...
sequence() {
    ...
    if [[ "${DOWNLOADER_WHEN_POSSIBLE}" == "true" ]] && [[ "${UPDATE_ALL_PC_UPDATER}" != "true" ]] ; then
        ...
        if [[ "${ARCADE_OFFSET_DOWNLOADER}" == "true" ]] ; then
            echo "- Arcade Offset folder by atrac17"
        fi
        if [[ "${COIN_OP_COLLECTION_DOWNLOADER}" == "true" ]] ; then
            echo "- Coin-Op Collection by atrac17"
        fi
...
run_update_all() {
    ...
    if [[ "${RUNNING_DOWNLOADER}" == "true" ]] ; then
        ...
        export COIN_OP_COLLECTION_DOWNLOADER="${COIN_OP_COLLECTION_DOWNLOADER}"
        ...
        (
            ...
            if [[ "${DOWNLOADER_WHEN_POSSIBLE}" == "true" ]] && [[ "${UPDATE_ALL_PC_UPDATER}" != "true" ]] ; then
                ...
                dialog --keep-window --default-item "${DEFAULT_SELECTION}" --cancel-label "Back" --ok-label "Select" --title "JTCORES for MiSTer Settings" \
                    ...
                    "2 Install Premium Cores" "$(settings_menu_yesno_bool_text ${DOWNLOAD_BETA_CORES})" \
                    "BACK"  "" 2> ${TMP}
                DEFAULT_SELECTION="$?"
                set -e
            else
...
settings_menu_2beta_arcade_organizer() {
    ...
    SETTINGS_OPTIONS_MAD_DB=("https://raw.githubusercontent.com/Toryalai1/MiSTer_ArcadeDatabase/db/mad_db.json.zip" "https://raw.githubusercontent.com/theypsilon/misteraddons_Arcade_MAD/db/mad_db.json.zip" "https://raw.githubusercontent.com/theypsilon/MAD_Database_MiSTer/db/mad_db.json.zip")
...
settings_menu_misc() {
    ...
    SETTINGS_OPTIONS_COIN_OP_COLLECTION_DOWNLOADER=("true" "false")
    ...
    while true ; do
        (
            ...
            local COIN_OP_COLLECTION_DOWNLOADER="${SETTINGS_OPTIONS_COIN_OP_COLLECTION_DOWNLOADER[0]}"
            ...
            load_vars_from_ini "$(settings_domain_ini_file ${EXPORTED_INI_PATH})" "AUTOREBOOT" "WAIT_TIME_FOR_READING" "COUNTDOWN_TIME" "DOWNLOADER_WHEN_POSSIBLE" "ARCADE_OFFSET_DOWNLOADER" "COIN_OP_COLLECTION_DOWNLOADER" "TTY2OLED_FILES_DOWNLOADER" "I2C2OLED_FILES_DOWNLOADER" "MISTERSAM_FILES_DOWNLOADER"
            ...
            if [[ "${DOWNLOADER_WHEN_POSSIBLE}" == "true" ]] && [[ "${UPDATE_ALL_PC_UPDATER}" != "true" ]] ; then
                ...
                dialog --keep-window --default-item "${DEFAULT_SELECTION}" --cancel-label "Back" --ok-label "Select" --title "Misc | Other Settings" \
                    ...
                    "3 Coin-Op Collection" "$(settings_menu_yesno_bool_text ${COIN_OP_COLLECTION_DOWNLOADER})" \
            ...
            case "${DEFAULT_SELECTION}" in
                ...
                "3 Coin-Op Collection") settings_change_var "COIN_OP_COLLECTION_DOWNLOADER" "$(settings_domain_ini_file ${EXPORTED_INI_PATH})" ;;
            ...
            case "${DEFAULT_SELECTION}" in
                ...
                "2 Play Bad Apple Database")
                    ...
                    if grep -q "fb_size=2" "${MISTER_INI_PATH}" 2> /dev/null || grep -q "fb_terminal=0" "${MISTER_INI_PATH}" 2> /dev/null ; then
                        export DEFAULT_DB_URL="https://github.com/theypsilon/BadAppleDB_MiSTer/releases/download/v1/bad_apple_half_res_db.json.zip"
                    else
                        export DEFAULT_DB_URL="https://github.com/theypsilon/BadAppleDB_MiSTer/releases/download/v1/bad_apple_full_res_db.json.zip"
                    fi
                    run_downloader_script "${DOWNLOADER_URL}" "Bad Apple DB!"
                    export -n DOWNLOADER_INI_PATH
